...
if [[ "$analysis" == "A45" ]]; then
	...
	GEN[$sample_id].GT GEN[$sample_id].AD[0] GEN[$sample_id].DP[0] | awk -F "\t" "BEGIN {OFS = FS} {gsub("/$sample_id/",\"index\",\$0); print \$0,\"na\",\"na\",\"na\"}" | sed 's/GEN\[index\].DP\[0\]\tna\tna\tna/GEN\[index\]\.DP\[0\]\tGEN\[mother\]\.GT\tGEN\[father\]\.GT\tGEN\[other\]\.GT/g' 2>$output_dir/logs/$family_id"_"$individual_id"_"$sample_id".dominant_"$analysis"_"$gene_list".log" > $output_dir/$family_id"_"$individual_id"_"$sample_id".dominant_"$analysis"_"$gene_list".txt"
